...
DEST_ROOT = UNITS_DIR.parent  # change to UNITS_DIR for in-place
...
# … existing tag assignment …
    if name == "Aircraft Plant":
        return f"Aircrafts/T{tier or 1}"
    
    if "aeroship" in name or "OPS Center" in name:
        return "Aeroship"

    if "hover" in name and tier in ("1", "2"):
        return f"Hovercraft/T{tier}"

    if "Experimental" in name:
        if "Gantry" in name:
            return f"Bots/T3"
        elif "Air Plant" in name:
            return f"Aircrafts/T3"
        elif "Vehicle" in name:
            return f"Vehicles/T3"

    if "prototype" in name:
        return f"Prototypes"
    
    if "lab" in name or "plant" in name or "platform" or "aircraft plant" in name or name.endswith("_ap"):
        if "infantry" in name:     return f"Kbot/T1"
        if "kbot" in name:     return f"Kbot/T{tier or '1'}"
        if "vehicle" in name:  return f"Vehicles/T{tier or '1'}"
        if "hover"   in name:  return f"Hover/T{tier or '1'}"
        if "aircraft" in name: return f"Aircrafts/T{tier or '1'}"
...
for uname, data in units.items():
    if any(x in uname for x in ("lab","plant","platform","_ap")):
        print(f"Tagging candidate: {uname} (static={data['static']})")
        # … existing tag assignment …
