return {
	armnebraska = {
		buildangle = 8192,
		buildcostenergy = 946189,
		buildcostmetal = 75932,
		builder = false,
		buildpic = "armnebraska.dds",
		buildtime = 750000,
		canattack = true,
		canstop = 1,
		category = "ALL SURFACE",
		collisionvolumeoffsets = "0 -60 0",
		collisionvolumescales = "215 180 215",
		collisionvolumetype = "ellipsoid",
		corpse = "dead",
		defaultmissiontype = "GUARD_NOMOVE",
		description = "Shield Piercing Gauss",
		explodeas = "MEDIUM_BUILDINGEX",
		firestandorders = 1,
		footprintx = 12,
		footprintz = 12,
		icontype = "building",
		idleautoheal = 5,
		idletime = 1800,
		losemitheight = 67,
		mass = 53932,
		maxdamage = 238450,
		maxslope = 10,
		maxwaterdepth = 0,
		name = "Nebraska",
		noautofire = false,
		objectname = "arm/armnebraska.s3o",
		radardistance = 0,
		radaremitheight = 66,
		selfdestructas = "MEDIUM_BUILDING",
		sightdistance = 1500,
		standingfireorder = 2,
		unitname = "armnebraska",
		yardmap = "oooooooooooo oooooooooooo oooooooooooo oooooooooooo oooooooooooo oooooooooooo oooooooooooo oooooooooooo oooooooooooo oooooooooooo oooooooooooo oooooooooooo",
		customparams = {
			buildpic = "armnebraska.dds",
			faction = "ARM",
			normaltex = "unittextures/arm_normals.dds",
		},
		featuredefs = {
			dead = {
				blocking = true,
				damage = 27385,
				description = "armnebraska Wreckage",
				featuredead = "heap",
				footprintx = 6,
				footprintz = 6,
				metal = 23812,
				object = "arm/armnebraska_dead.s3o",
				reclaimable = true,
				customparams = {
					fromunit = 1,
				},
			},
			heap = {
				blocking = false,
				damage = 34231,
				description = "armnebraska Debris",
				footprintx = 4,
				footprintz = 4,
				metal = 12700,
				object = "4x4d.s3o",
				reclaimable = true,
				customparams = {
					fromunit = 1,
				},
			},
		},
		sfxtypes = {
			pieceexplosiongenerators = {
				"piecetrail0",
				"piecetrail1",
				"piecetrail2",
				"piecetrail3",
				"piecetrail4",
				"piecetrail6",
			},
		},
		sounds = {
			canceldestruct = "cancel2",
			underattack = "warning1",
			cant = {
				"cantdo4",
			},
			count = {
				"count6",
				"count5",
				"count4",
				"count3",
				"count2",
				"count1",
			},
			ok = {
				"servlrg3",
			},
			select = {
				"servlrg3",
			},
		},
		weapondefs = {
			armnebraska_weapon = {
				areaofeffect = 180,
				avoidfeature = false,
				stockpile = true,
				stockpileTime = 5,
				cegtag = "GAUSS_HIT_L",
				collidefriendly = false,
				craterareaofeffect = 200,
				craterboost = 0,
				cratermult = 0,
				duration = 0.025,
				energypershot = 200000, -- incr /shot
				explosiongenerator = "custom:Tlldmc_Explosion",
				firestarter = 90,
				impactonly = 1,
				impulseboost = 0,
				impulsefactor = 0,
				intensity = 0.75,
				name = "Shield Piercing Gauss",
				nogap = 1,
				noselfdamage = true,
				range = 2300, -- basically 1.5x range
				reloadtime = 1,
				rgbcolor = "0 0.7 1",
				rgbColor2 = "0 1 1",
				soundhitdry = "xplolrg1",
				soundhitwet = "splslrg",
				soundhitwetvolume = 0.6,
				soundstart = "gauss5",
				turret = true,
				weapontype = "LaserCannon", -- changed to lasercannon to pass shields
				duration = 0.10,
				hardstop = false,
				thickness = 6.5, 
				weaponvelocity = 9000, -- tripleish velocity, again else this weapon is bad for a T5
				damage = {
					commanders = 3200,
					default = 42000, -- a little over 1.5x dmg
					subs = 5,
				},
				tracks = false,
			},
		},
		weapons = {
			{
				badtargetcategory = "HUGE LARGE MEDIUM SMALL MINOR",
				def = "armnebraska_WEAPON",
				onlytargetcategory = "SURFACE",
			},
		},
	},
}